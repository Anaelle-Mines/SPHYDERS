...
from Scenarios.scenarios_gasAnalysis import scenarioDict_gasAnalysis_Fr, scenarioDict_gasAnalysis_PACA
...
# for s in scenarioDict_gasAnalysis_Fr.keys():
   

#     inputDict = loadScenario(scenarioDict_gasAnalysis_Fr[s], False)
#     timeStep=scenarioDict_gasAnalysis_Fr[s]['timeStep'].loc[0,'timeStep']
...
dico={}
c=0
for scenar in scenarioDict_gasAnalysis_PACA.keys() :

    inputDict = loadScenario(scenarioDict_gasAnalysis_PACA[scenar], False)
    ...
    for k, v in scenarioDict_gasAnalysis_PACA[scenar].items():
        if type(v) is list:
    ...
    df=extract_energy(scenarioDict_gasAnalysis_PACA[scenar],'Marseille',outputFolder)[["SMR w/o CCUS","SMR + CCUS 50%","SMR + CCUS 90%","Alkaline electrolysis","PEM electrolysis","importsH2",'curtailment','totalProd','carbon','total_carbon','costs','total_costs']]
    df['scenario']=scenar
    df['order']=c
    c+=1
    dico[scenar]=df
