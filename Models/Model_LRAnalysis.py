...
from Scenarios.scenarios_LRAnalysis import scenarioDict_LRAnalysis_Fr, scenarioDict_LRAnalysis_PACA
...
# for s in scenarioDict_LRAnalysis_Fr.keys():
   

#     inputDict = loadScenario(scenarioDict_LRAnalysis_Fr[s], False)
#     timeStep=scenarioDict_LRAnalysis_Fr[s]['timeStep'].loc[0,'timeStep']
...
for scenar in scenarioDict_LRAnalysis_PACA.keys() :

    inputDict = loadScenario(scenarioDict_LRAnalysis_PACA[scenar], False)
    ...
    for k, v in scenarioDict_LRAnalysis_PACA[scenar].items():
        if type(v) is list:
    ...
    df=extract_energy(scenarioDict_LRAnalysis_PACA[scenar],'Marseille',outputFolder)[["SMR w/o CCUS","SMR + CCUS 50%","SMR + CCUS 90%",'feedBiogas',"Alkaline electrolysis","PEM electrolysis","importsH2",'importBM','curtailment','totalProd','carbon','total_carbon','costs','total_costs']]
    df['scenario']=scenar
    df['order']=c
    c+=1
    dico[scenar]=df
...
df.to_csv(outputPath+'results_LR.csv', index=True)
