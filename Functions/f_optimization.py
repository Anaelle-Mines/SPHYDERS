import pyomo
from pyomo.environ import Constraint, Set, Var
...
def getVariables_panda(model):
    ...
    for v in model.component_objects(Var, active=True):
        ...
        VAL = pd.DataFrame.from_dict(
            varobject.extract_values(), orient="index", columns=[str(varobject)]
        )
        ...
        if isinstance(varobject.index_set(), pyomo.core.base.set.SetProduct):
            ...
            DIM = (
                pd.DataFrame(VAL.index.tolist())
                .rename(columns=getSetNames(model, varobject.index_set()))
...
def getConstraintsDual_panda_indexed(model):
    ...
    for v in model.component_objects(Constraint, active=True):
        ...
        VAL = pd.DataFrame.from_dict(
            get_dualValues(model, cobject), orient="index", columns=[str(cobject)]
        )
        # print(get_set_name(model,varobject))
        if isinstance(cobject.index_set(), pyomo.core.base.set.SetProduct):
            ...
            DIM = (
                pd.DataFrame(VAL.index.tolist())
                .rename(columns=getSetNames(model, cobject.index_set()))
                .reset_index(drop=True)
            )
            VAL.reset_index(drop=True, inplace=True)
